...
After activation, the facilitator signs in at `/facilitator` and selects **Show invite code**. The browser requests the value on demand from the no-store, role-protected `/api/admin/invite` endpoint; it is not present in static assets or anonymous session responses. Select **Copy invite code**, then share it through an approved private channel and hide it before taking screenshots or screen sharing.
...
Local tests cover the async persistence contract, authorization boundaries, protected one-time bootstrap, single-facilitator invariant, existing participant journeys, the static Pages boundary and accessibility. Live evidence proves private Table/Blob readiness through managed identity, the public sign-in/join surface on desktop and mobile, and completed facilitator activation. Live participant registration, screenshot moderation and post-restart persistence remain pending. This workshop demo is not production certification.